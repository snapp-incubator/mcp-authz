...
// It depends only on net/http and gorilla/websocket to avoid pulling the heavy
// official Mattermost server module.
package mattermost
...
type Client struct {
	baseURL string // e.g. https://mattermost.snapp.cab
	token   string
	http    *http.Client
}

// NewClient builds a REST client. baseURL must have no trailing slash.
...
// Me returns the bot's own user (used to ignore the bot's own posts).
