...
// The bot calls this only after a query has been authorized, passing the user's
// identity and the namespaces they are allowed to query so the agent can scope
// itself.
package dify
...
type Client struct {
	baseURL string // e.g. https://dify.snappcloud.io/v1
	apiKey  string
	http    *http.Client
}
...
func NewClient(baseURL, apiKey string) *Client {
	return &Client{
		...
		apiKey:  apiKey,
		http:    &http.Client{Timeout: 5 * time.Minute}, // agent runs can be slow
	}
}
